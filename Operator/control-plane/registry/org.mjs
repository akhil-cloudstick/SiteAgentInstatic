...
export async function listOperators(scope) {
  if (isPlatform(scope)) {
    const { rows } = await query('select * from siteagent_control.operators order by name');
    return rows;
  }
  if (scope.level === 'operator') {
    const { rows } = await query('select * from siteagent_control.operators where id = $1', [scope.operatorId]);
    return rows;
  }
  // A Business admin sees its own Operator's name only through its Business.
  const { rows } = await query(
    `select o.* from siteagent_control.operators o
...
export async function getOperatorInScope(scope, id) {
  ...
  const { rows } = await query('select * from siteagent_control.operators where id = $1', [id]);
  return rows[0] || null;
}
...
export async function createOperator(scope, { name, slug }) {
  ...
  const { rows } = await query(
    'insert into siteagent_control.operators (slug, name) values ($1, $2) returning *',
    [s, n],
  );
  return rows[0];
}
...
export async function updateOperator(scope, id, { name }) {
  ...
  const { rows } = await query(
    'update siteagent_control.operators set name = $2, updated_at = now() where id = $1 returning *',
...
export async function getOperatorBySlug(slug) {
  const { rows } = await query('select * from siteagent_control.operators where slug = $1', [slug]);
  return rows[0] || null;
}
