...
// (colours, styles, fonts, scripts) — with an automatic timestamped backup.
// The running Instatic reads the site fresh from Postgres, so the change takes
...
writeFileSync(backupPath, JSON.stringify({ slug, site: siteRow?.settings_json ?? null, data_rows: allRows }))
console.log(`Backup: ${pagesBefore} content row(s) + site design -> ${backupPath}`)
...
console.log(`\n✅ Cleared "${slug}" CMS — pages ${pagesBefore} -> ${pagesAfter}, design reset to blank.`)
