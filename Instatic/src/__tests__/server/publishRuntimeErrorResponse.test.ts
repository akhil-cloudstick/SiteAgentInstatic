...
describe('publish runtime validation response', () => {
  it('returns live compiler diagnostics without attempting a publish', async () => {
    ...
    try {
      ...
      const siteResponse = await harness.cms('/cms/api/cms/site', { cookie })
      ...
      const response = await harness.cms('/cms/api/cms/runtime/validate', {
        method: 'POST',
        cookie,
        json: { site: draft },
      })
  ...
  it('returns an actionable 422 for an invalid authored runtime script', async () => {
    ...
    try {
      ...
      const response = await harness.cms('/cms/api/cms/publish', {
        method: 'POST',
        cookie,
      })
