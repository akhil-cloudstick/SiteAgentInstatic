...
export function getEditorCommands(): Command[] {
  return [
    ...
    {
      id: 'editor.deduplicateStyleRules',
      title: 'Clean up duplicate CSS tags',
      subtitle: 'Remove duplicate class and selector tags caused by multi-page imports',
      group: 'editor',
      iconName: 'eraser-solid',
      keywords: ['deduplicate', 'cleanup', 'duplicate', 'css', 'tags', 'styles', 'clean'],
      workspaces: ['site'],
      capability: SITE_WRITE_CAPABILITIES,
      run: async (ctx) => {
        ctx.closeSpotlight()
        const { useEditorStore } = await import('@site/store/store')
        const removed = useEditorStore.getState().deduplicateStyleRules()
        if (removed === 0) {
          console.info('[spotlight] No duplicate style rules found.')
        } else {
          console.info(`[spotlight] Removed ${removed} duplicate style rule(s).`)
        }
      },
    },
  ]
