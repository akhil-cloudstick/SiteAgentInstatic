...
import type { NodeTree, Page, PageNode, StyleRule, SiteDocument, PageTemplateConfig } from '@core/page-tree'
  ...
  function mutateAllPagesAndSite(
    ...
    return runHistoricMutation((draft) => {
      ...
      const helpers: SiteImportTransaction = {
        ...
        // --- MMS Share-to-CMS import transaction methods ---
        // Re-applied on top of upstream 0.0.16 helpers: OD owns the shared
        // chrome, so a re-share overwrites the everywhere template in place
        // rather than stacking duplicate layouts.
        upsertEverywhereTemplate({ title, slug, nodeFragment, priority = 0 }: { title: string; slug: string; nodeFragment: ImportFragment; priority?: number }): string {
          ...
          const graft = (page: Page): void => {
            for (const [id, node] of Object.entries(nodeFragment.nodes)) {
              page.nodes[id] = {
                ...node,
                classIds: linkImportedClassNames(node.classIds, site.styleRules, byName, allocateStyleRuleOrder),
              }
            }
            page.nodes[page.rootNodeId]!.children = [...nodeFragment.rootIds]
