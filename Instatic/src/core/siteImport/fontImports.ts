...
/**
 * Extract Google Fonts install requests from HTML `<link rel="stylesheet">`
 * hrefs. Reuses the same URL parser as extractGoogleFontImports so both
 * import paths produce identical self-hosted font results.
 */
export function extractGoogleFontsFromHtmlLinks(hrefs: string[]): ImportGoogleFont[] {
  const byFamily = new Map<string, ImportGoogleFont>()
  for (const href of hrefs) {
    const url = parseGoogleCss2Url(href)
    if (!url) continue
    for (const spec of url.searchParams.getAll('family')) {
      const font = parseFamilySpec(spec, url)
      if (font) mergeGoogleFont(byFamily, font)
