...
| **Shared components** | Two kinds. (1) Identical top-level `<nav>/<header>/<footer>` across ≥2 pages → one Visual Component each, hoisted into the everywhere layout ("Site Layout") and stripped from the pages, so new CMS pages inherit the chrome. (2) Any block marked **`data-shared="name"`** that occurs ≥2 times with identical structure → one Visual Component, with every occurrence replaced **in place** by a `base.visual-component-ref`, so the block keeps its position on the page and editing it once updates every occurrence. Both group by structural hash (active-state classes stripped), so copies must be byte-identical; `detectGlobalSections` / `detectSharedBlocks` in `globalSections.ts`. |
