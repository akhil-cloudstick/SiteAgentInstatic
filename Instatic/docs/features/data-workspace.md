...
## Visual contract — the approved MMSBUILD screen

The Data workspace is a pixel-match reproduction of the client-approved
**MMSBUILD Data Workbench** screen, the fourth screen in the MMSBUILD library
after Dashboard, Site and Content. Treat that screen as the specification.

**Token scope.** `AdminWorkspaceCanvasLayout` stamps `data-editor-screen="data"`
on the workspace body. `src/styles/globals.css` carries a matching
`[data-editor-screen='data']` block that transcribes the reference's `:root`
verbatim — palette, dark mirror, fixed `--data-sp-*` / `--data-text-*` scales,
and the geometry tokens. The attribute sits on the **body**, not the shell, so
the shared toolbar keeps the Dashboard palette; the re-skin is scoped to the
sidebar, grid and inspector. The block also re-points the generic tokens the
shared primitives read (`--bg-surface`, `--border`, `--input-radius`, …), which
is how `Button` / `Input` / `Dialog` pick up the reference's faces **without any
shared component being modified**.

**Fixed geometry, not fluid.** The reference specifies exact pixels — 78px rows,
54px header, 52px gutters, 44px tap targets, 8–9px badge text. The admin's fluid
`--space-*` / `--text-*` ramps cannot express those, so this screen uses the
numeric `--data-*` scales instead. The token is the delivery mechanism (the
spacing/typography policies forbid literal px in modules); the value is the
reference's.

**Icons.** This page renders the approved screen's original **pixel-art SVG**
artwork from `src/admin/pages/data/icons/`, generated by
`scripts/gen-data-icons.mjs`. The repo-wide `pixel-art-icons` vendor was
re-skinned to emit Remix webfont glyphs for the rest of the admin; the client
sign-off for Data is against the pixel-art artwork, so this page carries its own
copy. Data components import from `@admin/pages/data/icons`, never from
`pixel-art-icons/icons/*`.

**Deliberate deviations from the reference** (each is a correctness fix, not
drift — `src/__tests__/architecture/dataScreenFidelity.test.ts` documents and
guards them):

| Reference behaviour | What we do instead | Why |
|---|---|---|
| Publish split-button in a standalone 61px app header | Publish rides in the context bar's actions group | This admin has one shared toolbar across every workspace; the re-skin is body-scoped |
| Header's primary column is **not** sticky | Header and body primary cells share the same sticky offset | Otherwise header labels slide out over the wrong columns on horizontal scroll |
| Field-options menu absolutely positioned inside an `overflow: hidden` list | Portal `ContextMenu` | The reference's menu is clipped by its own container |
| Row menu clamped on X only | Clamp both axes | A bottom row's menu otherwise runs off-screen |
| `window.confirm()` for deletes | `useConfirmDelete` + step-up | `no-native-browser-dialogs`; the real app has re-auth |
| 3-role `ROLE_PROFILES` preview switcher | Real capability system | The mock's roles are a demo device |
| Session-only in-memory import | Real import/export endpoints | The mock has no server |
| 2-state sort | 3-state (asc → desc → none) | Adopting the mock's would remove "clear sort" |
...
single-line values (table names, breadcrumb, column headers, schema rows) or
`overflow-wrap: anywhere` for wrapping ones (meta values, record hero, notes).
Grid cells additionally clip (`overflow: hidden`) and use `.truncateCell`. This
is what keeps the layout aligned regardless of the data, and the fidelity gate
asserts it.

**Overflow rule.** `.scrollContainer` in `DataGrid.module.css` is the **only**
element permitted to scroll horizontally. The document must never gain a
horizontal scrollbar at any width; the grid is floored at
`--data-table-min-w` (760px) so narrow viewports scroll it rather than crushing
the columns.
...
| Hardcoding a colour, spacing or font-size in a Data CSS module | Add the value to the `[data-editor-screen='data']` block in `globals.css` and reference the token. `dataScreenFidelity.test.ts` and `css-token-policy.test.ts` both gate this. |
| Importing an icon from `pixel-art-icons/icons/*` inside `src/admin/pages/data/` | That renders the Remix webfont glyph, not the approved pixel-art artwork. Import from `@admin/pages/data/icons`. |
| Editing a shared CSS module's existing rule to restyle Data | Shared modules (`PanelRail`, `LeftSidebar`, `AdminCanvasLayout`, `RightSidebar`, `FloatingActionBar`) are used by Site / Content / Media. Only **append** a block guarded by `[data-editor-screen='data']`. |
| Changing `LEFT_SIDEBAR_DEFAULT_WIDTH` / `RIGHT_SIDEBAR_DEFAULT_WIDTH` to suit Data | Those constants are shared with Content and Media. Per-workspace starting widths live in `WORKSPACE_DEFAULT_WIDTHS` in `workspaceLayout.ts`. |
| Putting bundle Import / Export in the grid or the context bar | The approved screen keeps them in the sidebar panel header, and the reference's own contract test asserts the grid source contains neither string. |
