...
6. Calls `selectToolsForScope(scope, capabilities)` — write tools excluded without `ai.tools.write`.
7. Builds the scope system prompt (`buildSiteSystemPrompt(snapshot)` or `buildContentSystemPrompt(snapshot)`).
...
`site_render_snapshot` (and `site_read_document` / `site_get_node_html`) return large payloads. Seven rules keep them from exploding context (a screenshot inlined as base64 JSON text once pushed a single turn past 1M tokens):
...
6. **An image picks the model — per message, and it is never quietly dropped.** In managed mode the gateway resolves the model per request from the prompt's task-type category, and the tenant classifies on the prompt **text alone**. "Rewrite this heading" with a screenshot attached therefore classified as *Content*, whose model the operator wires for cheap text work — and the image was refused. The image is the part of a request that cannot be substituted, so it decides the route. `resolveImageRouting` (`server/ai/managed.ts`) returns two flags per message: `requiresVision` when **this turn** carries an image → the OpenRouter driver sends `x-instatic-ai-vision: 1` and the gateway's `resolveRoutedModel` upgrades the call to the *Design* category's multimodal model, overriding the classified category; and `routeReadsImages` (this turn has an image, or the text classified as `design`) which decides whether older images survive `projectUserImagesForModel` or fall back to breadcrumbs. **The decision is per message, never per conversation** — one thread can run `image + "build this"` → Design, then `"fix the wording"` → Content with earlier images elided, then `"restyle that section"` → Design with the images back. History is never mutated, so nothing is lost by the round trip; an unclassified turn is treated as text-only, because a weaker answer beats a refused request. Managed capabilities also stop claiming `toolResultImages` — the Responses `function_call_output` item is text-only, so a captured PNG was always downgraded to a note, and `render_snapshot` was paying the html-to-image cost for nothing.
7. **A refused image is recovered, not surfaced.** Rule 6 makes vision true by construction for managed mode, but declared capabilities can still be wrong — an operator can point Design at a text-only model, and OpenRouter's catalogue flag is *model*-level while its refusal is *endpoint*-level (a key whose provider/data-policy settings exclude the image-capable endpoints gets `404 No endpoints found that support image input` for a model the catalogue lists as multimodal). `classifyHttpFailure` gives that refusal its own `imageUnsupported` kind; the tool loop then drops **every** image — user turns (current one included, unlike the overflow retry) and tool-result attachments — and resends once. Unlike the overflow retry this is not gated on the initial round, because a round-2 screenshot trips it just as a round-1 attachment does. The loop also clamps `captureScreenshot` off for the rest of the run so later rounds stop re-attaching. Users only see an error if the image-free resend fails too.
