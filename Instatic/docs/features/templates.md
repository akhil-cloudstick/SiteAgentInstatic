...
- `resolveWrapperTemplates(site, doc)` (`src/core/templates/templateMatching.ts`) returns the templates that WRAP a document, outermost-first — the document-shaped counterpart to `resolveTemplateChain`'s route-shaped answer. A page, a `postTypes` template, or a `notFound` template ⇒ wrapped by the `everywhere` layout; the `everywhere` layout ⇒ nothing wraps it. Shared by the canvas and the draft preview (see "Preview composition" below), so both show the same chrome.
...
The toolbar's **Preview** overlay (`PreviewOverlay`) posts the in-memory draft to `POST /admin/api/cms/runtime/preview`, which renders it through `buildRuntimePreviewDocument`. That path composes the same way the public renderer does: `resolveWrapperTemplates(site, page)` → `composeTemplateChain(wrappers, { kind: 'page', page })` → `publishPage(merged, …)`. So the preview carries the `everywhere` layout's chrome (nav, footer) instead of showing the page's own nodes bare, and loop/media prefetch runs over the merged tree — a logo or menu loop inside the chrome resolves its data too.

Two deliberate asymmetries with the render:

- **Script selection stays keyed on the document, not the merged tree.** `collectRuntimeScripts` scopes each script by page, and composition rewrites the merged page's id/slug to the innermost template's. `buildSiteRuntimeScripts` therefore receives the original `page` — mirroring `publishSite`, which bundles per page and renders the merged tree.
- **Visual Component edit surfaces are never wrapped.** A `vc-virtual:<id>` page is not a published route, so the handler passes `wrapInTemplates: false` — the same branch `CanvasComposedTree` takes in VC mode.
