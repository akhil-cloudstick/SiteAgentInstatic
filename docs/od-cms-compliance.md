...
2. **Deterministic fix at share/preview — the guarantee.** Two mechanical, boilerplate incompatibilities that OD builds on nearly every page are repaired automatically so the page passes *clean* with **no tenant action and no gate block**:
   ...
   - It still does the older *lossless* normalize (inline CSS, convert utilities, wrap bare text, web-root asset paths). **Note on `wrapBareText`:** it is scoped to `h1`–`h6`/`p` that mix a wrapped word with loose text, and it only patches the **CMS-bound copy** — OD's source keeps the bare text, and the wrapper it inserts is a class-less `<span>` (normalize must never invent a design choice such as a class name). It is therefore *not* the fix for bare text: the **source** is corrected by the compliance loop instead, which runs `checkPageCompliance` on the **raw on-disk pages** after every chat run (`server.ts:collectCmsComplianceViolations`), so the bare-text `fail` is never masked by this transform. It does **not** silently rewrite genuine **design choices** (e.g. a `background: oklch(…)` colour) — those still FAIL the gate → Fix-it. **The line:** mechanical compatibility fixes are automatic (the page looks identical, it just imports); design choices are never silently changed.
...
- **Bare text** — a text run written directly inside a container (`<div>`, `<li>`, `<td>`, `<b>`, or a heading/paragraph that also holds a child element such as `<br>`/`<span>`). The importer keeps it as a no-wrapper `base.text` (`tag:'none'`) which emits **no DOM element at all**, so the tenant can never click it on the canvas, never gets a selection ring, and can never give it a class. Wrap every run in its own element **with its own class**. *(Checked as a hard `fail` — rule 8.)*
- **Text styled only through a descendant selector** (`.stat-item strong { … }`) — imports as an **ambient** rule: it renders, but editing it restyles every match instead of the one element the tenant selected, so per-element customisation is impossible. Give the element its own class and style that (`.stat-value { … }`). *(Checked as a hard `fail` — rule 16; severity is a one-line constant in both checkers if it proves noisy.)*
