...
- **Astro (build-time only for tenant sites):** Instatic builds *and* renders the **live** tenant site itself — there is no separate site **runtime** framework and **no headless content API**. Astro is still used to **build replica static sites that import into Instatic** (Super Import / "Path B" — see [`templateRule.md`](../../Operator/rules/templateRule.md)) and to build our own Operator Console. Astro is **not** the tenant runtime — once imported, Instatic owns rendering and publishing.
...
## AI models + usage attribution: one global key, per-task-type routing, per-tenant tagging
- One global **OpenRouter key** (Operator-set in the console). The model is **no longer a single global value**: the Operator defines **per-task-type categories** — built-in **Design** + **Content** plus custom ones — each mapped to its **own model**, with **exactly one Default** (used when a request is unclear or classification fails). A cheap **classifier model** reads each tenant prompt and picks the category; the gateway then resolves the concrete model **server-side** from that category and **never trusts a client-supplied `model`** (so enforcement can't be bypassed). The resolved model is echoed back (`x-instatic-resolved-model`) for per-tenant metering/audit.
- **Global AI guidance:** a standing plain-English instruction (authored in `rules/globalAiGuidanceRule.md`, served by the gateway `/config` probe) is injected into **every tenant's AI chat** so a non-technical owner's words become correct edits (turn phrasing → editor actions, respect the `templateRule.md` structure, clean up empty wrappers on removal, etc.). It is **file-authored + version-controlled**, not edited in the console. Category **model ids are never disclosed** to the tenant via `/config`; only the single Default model id is exposed (legacy `/model` probe).
- **AI Gateway** (control-plane proxy): each instance's OpenRouter base URL → `…/ai/<signed-token>/v1`; the gateway injects the global key, **resolves the per-category model (+ classifier)**, serves the task-type config + guidance, **tags + meters tokens/cost per tenant**, forwards to OpenRouter, feeds the dashboard. **Security property (required, whole project): the real key lives ONLY with admin in the gateway — it is never stored in any tenant instance, so neither a tenant nor their AI can read it.**
...
```
   operator/  —  Astro SSR console + control-plane services (on the VPS)
     settings(key · per-task-type models + classifier) · tenants+links · usage/health dashboards · alerts
...
1. **Operator Console (`operator/ui` + `operator/api`)** — Astro SSR: global settings (OpenRouter key + per-task-type AI models + classifier — saved separately from Cloudflare creds; global AI guidance is file-authored in `rules/`, not edited here), tenant CRUD + share links, usage/health dashboards (live islands), fleet overview, alerts.
...
4. **AI Gateway** — OpenRouter reverse proxy; injects global key, **routes each call to the operator's per-task-type model** (server-side category resolution + classifier) and injects **global AI guidance**, tags+meters per tenant, exposes usage to Metrics.
