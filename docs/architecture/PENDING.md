...
> Validated by importing a real Astro replica (**AtlasInfra**, built per [`templateRule.md`](../integration/templateRule-STALE-SNAPSHOT.md)) into a live tenant and diffing the published site against the original. Every gap was a **platform** bug — the replica was correct. All fixed in `instatic/vendor` (every tenant) + `operator/control-plane`.
