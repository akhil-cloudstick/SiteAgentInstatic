...
- **templateRule.md** — the build standard (`Operator/rules/templateRule.md`) that tells whoever rebuilds a client's old website exactly how to structure it (Astro static output, CSS inlined in HTML, JS from CDN, plain `<img>` tags, `data-sa` markers) so it imports into Instatic correctly and the Tenant gets full editing capability. SiteAgent does not enforce who follows this rule or which tool they use.
- **Site Replica** — a fresh rebuild of a client's existing website, produced by following `templateRule.md`. Delivered as a project folder (`dist/`). The Tenant imports it into their Instatic instance to begin editing.
- **Tenant site paths** — the two ways a Tenant builds a site: **(A)** build from scratch inside Instatic using the visual editor and AI agent; **(B)** import a Site Replica built with `templateRule.md` and then edit freely inside Instatic.
