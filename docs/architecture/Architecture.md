...
_The whole-project runtime picture. Terms per [`CONTEXT.md`](./CONTEXT.md); scope &amp; rationale per [`PLAN.md`](PLAN.md); database detail in [`DB-Architecture.md`](./DB-Architecture.md); review/spike history in [`PLAN-REVIEW-LOG.md`](./reviews/PLAN-REVIEW-LOG.md)._
...
- **Isolation = separate native processes + schema-per-tenant.** One **native Bun Instatic process** per Tenant (no Docker); one **shared Postgres database** with one **schema + least-privilege role** per Tenant. Cross-tenant data access is blocked by `GRANT`/`REVOKE`, **not** `search_path` (proven on real Postgres — see [`PLAN-REVIEW-LOG.md`](./reviews/PLAN-REVIEW-LOG.md) spike #9).
- **Off-the-shelf Instatic, native Bun.** Instatic (MIT, v0.0.7) runs as a **native Bun process** (`bun server/index.ts`, pinned to a commit) — **no Docker / WSL2 needed**; we never fork it. The agent, visual editor, drafts, RBAC, audit, and static publishing are all native to Instatic.
...
```
   ...
   ┌───────────────────────────────────────────────────────────────────────┐
   │  Tenant Instatic instances  (NATIVE Bun processes, one per tenant)     │
   │   instance:<port>  ── DATABASE_URL ─►  shared Postgres: schema <slug>   │
...
### 4. Tenant Instatic instances — `tenants/<slug>/` (native Bun processes)
One **native Bun Instatic process** per Tenant, spawned by the Provisioner via `child_process` (`bun server/index.ts`) with per-tenant env: its own `PORT`, `DATABASE_URL` (scoped to its schema), `UPLOADS_DIR`, and `INSTATIC_SECRET_KEY`. The built admin SPA (`STATIC_DIR = instatic/vendor/dist`) is shared across instances. Each runs the full Instatic feature set; the agent's OpenRouter base URL points at the AI Gateway. (Production adds Caddy/Traefik subdomain routing + auto-TLS; locally each instance is `127.0.0.1:<port>`.)
...
## Hosting tiers
- **Dev/test (current):** entirely local, **no Docker** — Node control-plane + Astro console + a **locally-installed Postgres** as the shared DB; tenants run as **native Bun processes** on localhost ports. Cloudflare free account for real publishes.
- **Production:** a **VPS (Linode)**: Console + control-plane + shared Postgres + N native Bun Instatic processes; Caddy/Traefik for editing-domain TLS; Cloudflare serves the public sites. Scale path: vertical first; then Postgres to a managed host + a connection pooler (one shared Postgres tops out around **~5–8 tenants** at default `max_connections=100`).

## Risks / open questions
- **Pre-alpha foundation** — Instatic v0.0.7 APIs churn; pin to a commit + budget breakage on upgrade.
- **Instatic schema-honoring — PROVEN LIVE.** A native instance migrated all **33 tables into the tenant schema, 0 in `public`** (see [`PLAN-REVIEW-LOG.md`](./reviews/PLAN-REVIEW-LOG.md)).
- **Process (not container) isolation** — weaker than containers; fine for local single-operator v1. Production can add containers/cgroups if desired.
...
## Review status
**v1 BUILT natively — no Docker.** Schema-per-tenant **proven live** (native Instatic → 33 tables in the tenant schema, 0 in `public`). Control-plane (Registry · Provisioner · TenantRuntime · AI Gateway · Publish Deployer) + Astro console implemented and running; provisioning **verified end-to-end** (Add Tenant → native Instatic boots, schema-isolated, serves the setup wizard). Publish path (`wrangler`) wired — needs the Operator's Cloudflare creds in Settings to exercise. Two independent plan reviews complete; all findings folded or consciously accepted.
