...
1. Someone has rebuilt the client's old website following `Operator/rules/templateRule.md` — the output is a project folder (e.g. `dist/`) built with Astro, with CSS inlined in the HTML, JS libraries from CDN, and images in `public/`.
