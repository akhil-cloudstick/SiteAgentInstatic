...
> **Companion docs**
> - `docs/opendesign-0.16.1-upgrade-report.md` — the **customization ledger** (what's MMS-owned + how each conflict was resolved). Clone and re-verify it each upgrade; it's the single most useful artifact.
> - `docs/opendesign-version-comparison.md` — human summary of what each release changed (regenerate per upgrade).
> - `docs/instatic-upgrade-runbook.md` — the sibling procedure for the CMS. Same golden rule, different repo shape.
...
`docs/opendesign-shared-web-build.md`. `--force` is required after a source upgrade; without it the
...
- Keep the ledger (`docs/opendesign-<version>-upgrade-report.md`) current — clone it per upgrade and
  re-verify every row.
