...
- **[`docs/integration/od-cms-compliance.md`](./od-cms-compliance.md)** — the core compliance logic (veg-kitchen: OD only builds what the CMS can import), the researched accept/block matrix, and the **future-failure playbook** (diagnose adherence-gap vs coverage-gap → fix → test). Read it before touching templateRule / the CMS contract / the gate.
