...
### Flow 4 — Import &amp; edit own HTML (Super Import)
1. Tenant drags in an existing HTML/CSS site (the commit step runs in their browser).
2. Plain pages import cleanly and become editable. **Heavy-animation pages** keep their motion as an opaque block — editable around it, but the animated parts shouldn't be restructured. *(Fidelity confirmed by a dedicated spike before relying on it.)*
