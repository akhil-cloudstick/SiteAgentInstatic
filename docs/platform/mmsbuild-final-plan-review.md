...
| **Import Engine** (Firecrawl + visual-reconstruction) | Automatically reads an old website and pulls out its content | **Doesn't exist.** Today, rebuilding an old site is done **by hand** (a person or an AI agent follows `Operator/rules/templateRule.md` and rebuilds the site), then it's imported into Instatic. That import step works — the automatic "read the old site for me" step doesn't exist yet. |
...
**Where we stand:** we already did a version of this for SiteAgent itself (`docs/architecture/PLAN.md` and friends) — but not yet for MMSBUILD's new pieces (Firecrawl, fal.ai, Approval Engine, Report Engine).
