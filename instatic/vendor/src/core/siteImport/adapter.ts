...
export interface SiteImportTransaction {
  ...
  /**
   * Create a new Visual Component from an import fragment and return its id.
   *
   * The `nodeFragment` is the normalised representative sub-tree from a
   * cross-page global section detection pass. Class names inside are still raw
   * name strings (same semantics as `addPage`'s nodeFragment — the implementer
   * reconciles names to registry ids). The new VC starts with no params and an
   * empty classIds array.
   *
