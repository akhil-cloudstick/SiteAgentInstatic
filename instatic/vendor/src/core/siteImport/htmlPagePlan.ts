...
interface HtmlPagePlanResult {
  ...
  inlineCss: string
  /**
   * External `https://` stylesheet hrefs from `<link rel="stylesheet">` tags
   * that couldn't be resolved to a FileMap path (e.g. Google Fonts CDN links).
   * `buildImportPlan` feeds these to `extractGoogleFontsFromHtmlLinks` so the
   * font is self-hosted rather than left as an external CDN dependency.
   */
...
export function makeHtmlPagePlan(
  ...
  for (const href of linkHrefs) {
    ...
    if (resolved && fileMap.files[resolved]) {
      ...
      // External CDN stylesheet (e.g. Google Fonts) — collected so the caller
      // can install the font self-hosted rather than leaving a CDN dependency.
  ...
  return { pagePlan, warnings, inlineCss: styleCss, externalLinkHrefs }
}
