...
export function cssToStyleRules(
  ...
  try {
    ...
    //
    // Minifiers omit the space between an at-keyword and the opening paren —
    // e.g. `@media(min-width:1024px)` — which is valid CSS per spec but causes
    // happy-dom to silently drop the rule.  Normalise before handing to the
    // parser: `@media(` → `@media (`, same for @supports and @container.
