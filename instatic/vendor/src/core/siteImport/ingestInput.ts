...
async function ingestFiles(
  ...
  // When the user picks a folder via webkitdirectory, every path is prefixed
  // with the folder name (e.g. `dist/index.html`). Strip it the same way ZIP
  // ingestion does, so slugs become `/blog` not `/dist/blog`.
  const allPaths = Object.keys(rawFiles)
  const topLevel = detectSharedTopLevel(allPaths)
