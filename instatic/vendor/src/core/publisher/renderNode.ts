...
function renderStandardNode(
  ...
  let resolvedProps = resolvePageRefProps(dynamicProps, config.site.pages)

  // For anchor modules, inject aria-current="page" when the link's href
  // matches the current route path. This makes shared navbar components
  // highlight the correct item on every page without any per-page authoring.
  if (def.htmlTag === 'a') {
    const routePath = config.templateContext?.route?.path
    const href = resolvedProps.href
    if (routePath && typeof href === 'string' && href.length > 0) {
      const normalize = (p: string) => p.replace(/\/$/, '') || '/'
      if (normalize(href) === normalize(routePath)) {
        const existing = resolvedProps.htmlAttributes
        resolvedProps = {
          ...resolvedProps,
          htmlAttributes: {
