...
import { indexStyleRulesByName, indexAmbientRulesBySelector, linkImportedClassNames } from './importLinking'
...
export function buildSiteHelpers(
  ...
  function mutateAllPagesAndSite(
    ...
    return runHistoricMutation((draft) => {
      ...
      const ambientBySelector = indexAmbientRulesBySelector(site.styleRules)
      ...
      const helpers: SiteImportTransaction = {
        ...
        addStyleRule(rule: NewStyleRule): string {
          // Astro (and any SSG) inlines the same <style> block into every
          // page's HTML. Processing N pages produces N identical CSS rules per
          // class. byName / ambientBySelector already contain every rule added
          // earlier in this recipe, so returning the existing id here prevents
          // duplicates for both class rules and ambient selectors (e.g. `a`,
          // `.nav-link:hover`).
          if (rule.kind === 'class') {
            const existing = byName.get(rule.name)
            if (existing) return existing
          } else {
            const existing = ambientBySelector.get(rule.selector)
            if (existing) return existing
          }
          const id = nanoid()
          const now = Date.now()
          ...
          else ambientBySelector.set(rule.selector, id)
          didMutate = true
          return id
        },
        ...
        createVisualComponent({ name, nodeFragment }): string {
          const vcId = nanoid()
          const rootNodeId = nodeFragment.rootIds[0]
          if (!rootNodeId) return vcId
          const now = Date.now()
          // Reconcile class names → registry IDs, same as addPage does for
          // page nodes, so the VC's nodes reference real class IDs that the
          // editor can look up in site.styleRules.
          const reconciledNodes: Record<string, VCNode> = {}
          for (const [id, node] of Object.entries(nodeFragment.nodes)) {
            reconciledNodes[id] = {
              ...node,
              classIds: linkImportedClassNames(node.classIds, site.styleRules, byName),
            } as unknown as VCNode
          }
