...
/**
 * A top-level semantic section (`<nav>`, `<header>`, `<footer>`) that appears
 * structurally identical across two or more imported pages. Detected by
 * `detectGlobalSections` in `buildPlan.ts` after the per-page plans are built.
 *
...
export interface GlobalSectionCandidate {
  ...
  tag: string
  /** Deterministic hash of the normalised node tree (active-state classes removed). */
  hash: string
  /** FileMap paths of pages that contain this section. */
  pageSources: string[]
  /**
   * The original top-level node ID of the section in each page's nodeFragment,
   * keyed by page source path. Used during commit to locate and replace the node.
   */
  rootIdByPageSource: Record<string, string>
  /**
   * Normalised sub-fragment (active-state classIds stripped) taken from the
   * first matching page. Used as the VisualComponent's node tree on commit.
   */
  representativeFragment: ImportFragment
  /**
   * True when at least one `base.link` node in the section had an active-state
   * class stripped during normalisation. On commit, triggers injection of a
   * runtime active-state script for every page that uses the shared section.
   */
...
export interface ImportPlan {
  ...
  unusedCss: string[]
  /**
   * Top-level semantic sections (`<nav>`, `<header>`, `<footer>`) detected as
   * structurally identical across ≥2 imported pages. Each will be promoted to a
   * VisualComponent by `commitImportPlan`, and each affected page's section node
   * replaced with a `base.visual-component-ref`.
   *
