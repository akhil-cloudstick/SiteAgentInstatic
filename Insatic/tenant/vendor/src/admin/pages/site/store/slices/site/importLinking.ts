...
/**
 * Index ambient (non-class) style rules by selector → id.
 * Used alongside `indexStyleRulesByName` to prevent duplicate ambient rules
 * (e.g. `a`, `.nav-link:hover`) within a single import batch.
 */
export function indexAmbientRulesBySelector(rules: Record<string, StyleRule>): Map<string, string> {
  const bySelector = new Map<string, string>()
  for (const r of Object.values(rules)) {
    if (r.kind !== 'class' && !bySelector.has(r.selector)) bySelector.set(r.selector, r.id)
  }
  return bySelector
}
