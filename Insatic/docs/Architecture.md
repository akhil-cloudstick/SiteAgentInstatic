...
- **Isolation = separate native processes + schema-per-tenant.** One **native Bun Instatic process** per Tenant (no Docker); one **shared Postgres database** with one **schema + least-privilege role** per Tenant. Cross-tenant data access is blocked by `GRANT`/`REVOKE`, **not** `search_path` (proven on real Postgres — see [`PLAN-REVIEW-LOG.md`](PLAN-REVIEW-LOG.md) spike #9).
...
- **Instatic schema-honoring — PROVEN LIVE.** A native instance migrated all **33 tables into the tenant schema, 0 in `public`** (see [`PLAN-REVIEW-LOG.md`](PLAN-REVIEW-LOG.md)).
