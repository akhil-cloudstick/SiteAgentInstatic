...
export function registerStaticResourceRoutes(app: Express, ctx: RegisterStaticResourceRoutesDeps) {
  ...
  app.get('/api/skills', async (req, res) => {
    try {
      ...
      const workspaceMemberIdForScope = authority?.workspaceMemberId ?? null;
      // MMS: this listing walks the skills root on disk, which is ruinous over
      // the SMB working drive. Cache per workspace scope; the cache generation
      // is bumped by any skill write (see read-cache.ts).
      const skills = await cachedRead(
        `skills:all:${workspaceId ?? ""}|${workspaceMemberIdForScope ?? ""}`,
        () => listAllSkills({ workspaceId, workspaceMemberId: workspaceMemberIdForScope }),
      );
  ...
  app.get('/api/design-systems', async (req, res) => {
    try {
      ...
      // MMS: same SMB rationale as the skills catalog above.
      const catalog = await cachedRead(
        `design-systems:all:${workspaceId ?? ""}|${workspaceMemberId ?? ""}`,
        () => listAllDesignSystems({ workspaceId, workspaceMemberId }),
      );
