...
export function registerPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.get('/api/plugins', async (req, res) => { try { const authority = await resolveWorkspaceAuthority(req, res, deps.verifyWorkspaceReadAuthority ?? deps.verifyWorkspaceRequestAuthority); if (authority === undefined) return; const scopeKey = `${authority?.workspaceId ?? ''}|${authority?.workspaceMemberId ?? ''}`; res.json({ plugins: await cachedRead(`plugins:installed:${scopeKey}`, async () => helpers.applyBakedPreviews(await plugins.listInstalledPlugins(db, authority?.workspaceId ?? null, authority?.workspaceMemberId ?? null), helpers.PLUGIN_PREVIEWS_DIR)) }); } catch (err) { res.status(500).json({ error: String(err) }); } });
