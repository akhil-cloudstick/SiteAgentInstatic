...
export function registerPluginRoutes(app: Express, deps: RegisterPluginRoutesDeps): void {
  ...
  app.get('/api/plugins', async (_req, res) => { try { res.json({ plugins: await cachedRead('plugins:installed', async () => helpers.applyBakedPreviews(plugins.listInstalledPlugins(db), helpers.PLUGIN_PREVIEWS_DIR)) }); } catch (err) { res.status(500).json({ error: String(err) }); } });
