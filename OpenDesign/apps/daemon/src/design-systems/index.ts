...
export async function listDesignSystems(
  ...
  const collected = await Promise.all(entries.map(async (entry) => {
    ...
    try {
      ...
      // MMS fans these reads out via Promise.all(map()) rather than a for-loop
      // (network share), so upstream loop-body `continue` becomes `return null`.
      ...
      return {
        ...
        ...(metadata.teamSynced ? { teamSynced: true as boolean } : {}),
