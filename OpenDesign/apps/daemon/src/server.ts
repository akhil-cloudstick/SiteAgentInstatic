...
export async function startServer({
  ...
  if (tenantSsoEnabled()) {
    app.use((req, res, next) => {
      ...
      // `od_session` is short-lived and absent entirely on a bookmark or a fresh
      // browser. Rendering our own "sign in required" page here is the second
      // login users report. When the control plane tells us where the hub is,
      // bounce a top-level navigation there instead: it silently re-mints an SSO
      // token if the hub session is alive, and shows the ONE shared login only
      // when it is not. Sub-resource requests still get a plain 401 — a redirect
      // is meaningless to fetch/XHR.
      const hubSsoUrl = (process.env.OD_HUB_SSO_URL ?? '').trim();
      const isDocument = req.get('sec-fetch-dest') === 'document'
        || (!req.get('sec-fetch-dest') && (req.get('accept') ?? '').includes('text/html'));
      if (hubSsoUrl && isDocument) {
        const next = req.originalUrl || req.path;
        res.redirect(302, `${hubSsoUrl}?next=${encodeURIComponent(next)}`);
        return undefined;
      }
      res
        .status(401)
  ...
  async function computePluginRegistryView(options: { workspaceId?: string | null; workspaceMemberId?: string | null } = {}) {
  ...
  async function loadPluginRegistryView(
    ...
    // MMS: the cache below keys only on the skill/design-system folder
    // signature, so it is valid ONLY for the unscoped view. A workspace-scoped
    // request (upstream team edition) must neither read nor fill that cache.
    if (options.workspaceId !== undefined) return computePluginRegistryView(options);
    const now = Date.now();
    ...
    const value = await computePluginRegistryView(options);
  ...
  const composeDaemonSystemPrompt = async ({
    ...
    // choice and is respected (allowAppDefault stays project===null). Web Clone
    // runs stay exempt even in tenant mode: reproducing someone else's site
    // must not repaint it in the tenant's brand.
    const instaticTenantMode =
      !!(process.env.OD_INSTATIC_URL ?? '').trim() && tenantSsoEnabled();
    let designSystemSelection = isWebCloneRun
      ? { id: null, source: 'none' }
      : resolveEffectiveDesignSystemSelection({
          ...
          // "No design system"; do not reapply the global default behind their
          // back — except in tenant mode, where a bound brand is non-negotiable.
    ...
    if (instaticTenantMode && !isWebCloneRun && !designSystemSelection.id) {
    ...
    const systemPromptInputs = {
      ...
      mediaHintSignal,
      platformHintSignal,
      // MMS: upstream v0.16.1 ships this defaulting to 'slim', but that is a
      // VALIDATION-ONLY flip carried in from its feat/system-prompt integration
      // branch — upstream's own comment reads "main keeps classic as the default
      // — do NOT carry this flip into a PR against main". We adopt the slim
      // plumbing but keep CLASSIC as the default, because:
      //   • the CMS output contract + templateRule compliance gate were tuned
      //     against the classic stack, and a silent prompt-core swap changes
      //     what Share-to-CMS produces;
      //   • slim deliberately skips the tail overrides, including
      //     ACTIVE_DESIGN_SYSTEM_VISUAL_DIRECTION_OVERRIDE, which backs the
      //     mandatory tenant brand resolved above.
      // Opt in per-daemon with OD_PROMPT_CORE=slim once it has been validated
      // against the compliance gate.
  ...
  async function runCmsComplianceGate(sourceRun) {
    try {
      ...
      for (let attempt = 1; attempt <= CMS_COMPLIANCE_MAX_ATTEMPTS; attempt++) {
        ...
        // Some rules have a remedy that is NOT obvious from the finding alone,
        // and guessing it destroys the page. The external-stylesheet finding is
        // the worst offender: an agent told only "External stylesheet: <link …>"
        // satisfies the rule by DELETING the link, which strips every shared
        // header/nav/footer rule and ships an unstyled page that still "passes".
        // So state the required remedy explicitly for the rules that need one.
        const remedies = [];
        if (violations.some((v) => v.fails.some((f) => f.rule === 'CSS inline in <style>'))) {
          remedies.push(
            'For "CSS inline in <style>": COPY THE FULL CONTENTS of the referenced stylesheet ' +
              'into a <style> block in that page, THEN remove the <link>. Do NOT delete the ' +
              '<link> without inlining it — the shared header/nav/footer depend on those rules ' +
              'and the page will render unstyled. Put the shared CSS FIRST so the page\'s own ' +
              'styles, which come after, still win where they overlap.',
          );
        }
        if (violations.some((v) => v.fails.some((f) => f.rule === 'Styles present for markup'))) {
          remedies.push(
            'For "Styles present for markup": the page uses classes that no CSS in the page ' +
              'defines — a stylesheet was dropped without inlining it. Restore the missing rules ' +
        ...
        const correctionMessage = [
          ...
          summary,
          ...(remedies.length ? ['', 'How to fix (follow exactly):', ...remedies.map((r) => `- ${r}`)] : []),
        ].join('\n');
