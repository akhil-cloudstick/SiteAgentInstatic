...
// Behind the SiteAgent gateway the app is served under a FIXED `/design`. Raw-file
// URLs are used both as preview `<base href>` values (resolved by a sandboxed,
// opaque-origin iframe that carries no Referer) and as fetch targets, so they
// MUST carry the `/design` prefix to reach the tenant's daemon through the gateway.
// The gateway maps that to the right daemon using the hub session cookie — the
// prefix is deliberately tenant-agnostic so one build serves every tenant.
// Served at the root (local dev) this is a no-op.
