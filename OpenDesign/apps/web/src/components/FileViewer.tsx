...
function FileVersionManagerModal({
  ...
  async function captureVersionPreviewSnapshot(options?: { full?: boolean }) {
    ...
    await waitForAnimationFrame();
    await waitForAnimationFrame();
    // `requestPreviewSnapshotWithRetry` returns the richer MMS PreviewSnapshotResult
    // (so the annotate path can distinguish render-error from still-loading and
    // stop retrying). Upstream's export callers just want the snapshot or null,
    // so unwrap it here rather than weakening the result type.
    if (options?.full) {
      const full = await requestPreviewSnapshotWithRetry(iframe, options);
      return full.ok ? full.snapshot : null;
    }
    const hostSnapshot = await captureHostIframeSnapshot(iframe);
    if (hostSnapshot) return hostSnapshot;
    const result = await requestPreviewSnapshotWithRetry(iframe, options);
