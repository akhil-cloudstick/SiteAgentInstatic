...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the MMS Design community catalog at `nexu-io/open-design`.',
...
// MMS Design issue URL and belongs to the "Open Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an MMS Design issue URL and belongs to the "Open Design PR" button.',
    '',
    'Run this deterministic Open Design CLI workflow from the current project workspace:',
    '',
