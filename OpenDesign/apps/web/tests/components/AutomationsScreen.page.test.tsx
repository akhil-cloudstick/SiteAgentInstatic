...
function mockAutomationsScreenFetch({ routines = [] }: { routines?: Routine[] } = {}) {
...
describe('AutomationsScreen page shell', () => {
  afterEach(() => {
    cleanup();
  ...
  it('renders the automations page hero and summary metrics', async () => {
    ...
    mockAutomationsScreenFetch({ routines });
  ...
  it('shows the template empty state when switching to an empty category', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('status').textContent ?? '').toContain('No templates in this filter');
    });
  ...
  it('pauses and resumes an automation through PATCH updates', async () => {
    ...
    const row = (await screen.findByText('Daily digest')).closest('li')!;
    // Pause / Resume live behind the reference's `⋯` row menu, which closes
    // on selection — so each toggle re-opens it.
    const openRowMenu = () =>
    ...
    await waitFor(() => expect(row.getAttribute('data-paused')).toBe('true'));

    openRowMenu();
    expect(within(row).getByRole('button', { name: 'Resume' })).toBeTruthy();
    fireEvent.click(within(row).getByRole('button', { name: 'Resume' }));
    await waitFor(() => expect(row.getAttribute('data-paused')).toBe('false'));
  ...
  it('deletes an automation after confirmation and returns to the empty state', async () => {
    ...
    const row = (await screen.findByText('Daily digest')).closest('li')!;
    // Delete is a labelled `menuitem` in the reference's `⋯` row menu.
    fireEvent.click(within(row).getByRole('button', { name: /More actions for/i }));
    fireEvent.click(within(row).getByRole('button', { name: 'Delete' }));

    await waitFor(() => {
      // The reference's empty state is a static block, not a button.
