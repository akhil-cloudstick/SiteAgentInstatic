...
import { AutomationComposeModal } from '../../src/components/AutomationComposeModal';
// The template shape moved out of the modal into its own module when the screen
// was rebuilt against the approved reference.
import type { AutomationTemplate } from '../../src/components/automationTemplates';
...
describe('AutomationComposeModal context picker', () => {
  ...
  it('picks skills, plugins, MCP servers, and connectors from @ in the prompt', async () => {
    ...
    // CSS-Module class names are hashed, so the mention state is exposed as a
    // data attribute on the reference's `.automation-prompt-field` label.
    const promptField = prompt.closest('[data-testid="automation-modal-prompt-field"]');
    expect(mentionPopover).toBeTruthy();
    expect(promptField?.getAttribute('data-mentioning')).toBe('true');
    // The picker is an overlay inside the compose body, never a child of the
    // prompt field — otherwise it would be clipped by the field's own box.
    expect(mentionPopover.closest('[data-testid="automation-modal-prompt-field"]')).toBeNull();
