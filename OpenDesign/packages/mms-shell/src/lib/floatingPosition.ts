...
export function computeFloatingPosition(
  ...
  // `scored` is built from a non-empty candidate list, but MMS Design compiles
  // this under a stricter null policy than MMS-CMS — assert the invariant
  // rather than let the two products disagree about whether this file is valid.
  const best = scored[0]?.s ?? 'bottom'
