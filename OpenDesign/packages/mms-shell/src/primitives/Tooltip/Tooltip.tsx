...
function TooltipInner({
  ...
  // `useCallback`, not React 19's `useEffectEvent`: this primitive is shared
  // with a React 18 host (MMS Design), where that export does not exist. The
  // three setters are stable, so the empty dep list gives the same
  // never-changing identity `useEffectEvent` provided — which is what the
  // effects below rely on. (One of the three sanctioned exceptions to the
  // no-manual-memoization rule: the value feeds a dependency array.)
