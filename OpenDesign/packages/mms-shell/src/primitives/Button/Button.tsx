...
/**
 * `forwardRef`, not React 19's ref-as-prop: this primitive is shared with a
 * React 18 host (MMS Design / OpenDesign), where a plain `ref` prop is stripped
 * before it reaches the component and every anchored menu built on it silently
 * loses its trigger. `forwardRef` behaves identically on both majors.
 */
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  ref,
    ...
    // `process.env.NODE_ENV` rather than `import.meta.env.DEV`: this file is
    // compiled by Vite in the CMS and by Turbopack in MMS Design, and only the
    // former defines `import.meta.env`. Both replace NODE_ENV at build time.
    ...
    const button = (
      <button
        ...
        onClick={effectiveAriaDisabled ? (e: MouseEvent<HTMLButtonElement>) => e.preventDefault() : onClick}
      >
        {children}
      </button>
    );
