...
import { collectSameOriginDocuments, isNode } from '../../lib/sameOriginDocuments'
...
interface ContextMenuProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
  ...
  animateExit?: boolean
  /**
   * Dismiss when the page scrolls under the menu, instead of re-anchoring to
   * the trigger.
   *
   * Default `false` — a menu opened over scrollable content (canvas, file
   * list) should follow its trigger, which is what `useAnchorPosition` does.
   * Opt in for navigation chrome, where a scroll means the user has moved on
   * and a menu riding along on top of the content reads as stuck.
   */
...
export const ContextMenu = forwardRef<HTMLDivElement, ContextMenuProps>(function ContextMenu({
  ...
  // Opt-in dismissal on page scroll.
  //
  // The default is to STAY OPEN and re-anchor (see `useAnchorPosition`), which
  // is right for a menu opened over content the user may need to scroll — a
  // canvas, a long file list. It is wrong for navigation chrome: scrolling the
  // page is the user moving on, and a nav menu that rides along on top of the
  // content reads as stuck. Off by default so no existing menu changes.
  useEffect(() => {
    if (!closeOnScroll) return undefined
    function handleScroll(event: Event) {
      // Scrolling INSIDE the menu is the user reading a long list, not leaving.
      const target = event.target
      if (isNode(target) && menuRef.current?.contains(target)) return
      beginClose()
    }
    // Capture, because the element that scrolls is usually a descendant of
    // document and scroll does not bubble.
    document.addEventListener('scroll', handleScroll, true)
    return () => document.removeEventListener('scroll', handleScroll, true)
  }, [closeOnScroll, beginClose])
